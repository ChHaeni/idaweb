...
#' @param force_cache logical. Should the argument \code{cache_dir} be taken, even if the cached data has been saved elsewhere during the current R session? Default \code{FALSE}.
...
#' @details argument \code{outstruc} can be used to structure the output in four different ways.
#'      \code{\sQuote{split-all}} (default) \code{list} with each entry representing unique combinations of station/granularity.
#'      \code{\sQuote{by-station}} gathers all granularities for each unique stations as single list entries.
#'      \code{\sQuote{by-granularity}} gathers all stations for each unique granularity.
#'      \code{\sQuote{cbind-all}} gathers all data into a single object.
